...
public class ALGraph{
		...
		public void traverseBFS(String vertexName){
				if(vertexList.head == null){
					System.out.println("Graph has no vertices!");
					return;	
				}
				if(!vertexList.exists(vertexName)){
						return;
				}

				VertexQueue vQueue = new VertexQueue();
				vQueue.enqueue(vertexName);
				vertexList.visit(vertexName);
				while(!vQueue.isEmpty()){
						String curr = vQueue.dequeue();
						System.out.print(" "+curr+" ");
						String[] adj = listAdjacentVertices(curr).split(" ");
		...
		private boolean checkAcyclic(){
				return vertexList.checkAcyclic();
		}
		public String[] topologicalSort(ALGraph graph) throws CyclicException{
				if(graph.checkAcyclic() == false){
		...
		private class VertexList{
				...
				private void deleteVertex(String vertexName){
						...
						if(head.getName().equals(vertexName)){
								edgeCount  = edgeCount - head.getLength();
								head = head.next;
								System.out.println("reduced edgeCount by: "+head.getLength());
						}
						else{
								Vertex curr =  head;
								while(curr != null){
										if(curr.next != null && vertexName.equals(curr.next.getName())){
												edgeCount  = edgeCount - curr.next.getLength();
												System.out.println("reduced edgeCount by: "+curr.next.getLength());
												break;
										}
										curr = curr.next;
								}	
						...
						Vertex curr = head;
						while(curr != null){
								if(curr.checkAdjacent(vertexName)){
				...
				private boolean checkAcyclic(){
						Vertex curr = head;
						while(curr != null){
								if(checkConnected(curr.getName(), curr.getName())){
										return false;
								}
								curr = curr.next;
						}
